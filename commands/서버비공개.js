...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('서버비공개')
        .setDescription('로블록스 서버를 비공개로 설정합니다.'),
    ...
    async execute(interaction) {
        ...
        try {
            const success = await setGameVisibility(config.UNIVERSE_ID, false);
            
            if (success) {
                return interaction.editReply(`🔒 로블록스 서버가 비공개로 설정되었습니다!`);
            } else {
                return interaction.editReply({ content: `❌ 서버 비공개 설정에 실패했습니다.` });
            }
        } catch (error) {
            console.error('서버 비공개 설정 오류:', error);
